...
The loader builds an individual dispatch table for each `XrInstance` that is
created.
...
creation.
For OpenXR API commands which don't include an `XrInstance` explicitly, the
loader uses the stored parent information to identify the appropriate call
chain to be dispatched.
