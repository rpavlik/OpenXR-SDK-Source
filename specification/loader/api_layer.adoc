...
If an OpenXR command is not intercepted by an API layer, or if an API layer
chooses to terminate the function by not calling down the chain, then no
dispatch is needed for that particular function.
